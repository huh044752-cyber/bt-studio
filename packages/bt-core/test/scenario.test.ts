...
import { parseScenarioUnits, attachTreeToScenario, validateTreeForUnit, attachOldScenario, attachOldScenarioStateMachine } from "../src/index.js";
...
<Scenario>
  ...
  it("attachOldScenarioStateMachine:把 ADD StateMachine 与 ADD Behaviac 平行写入 <Unit><Script>", () => {
    // 老引擎 fosim_script_ast::ST_AddStateMachine 与 ST_AddBehaviac 是平行命令,想定挂载走同一个 <Script> 通道。
    // 用户报的"FSM 挂靠没成功"就是原先脚本把 FSM 塞进 <BehaviorLogic>(老引擎不看)而不是 <Script>ADD StateMachine 命令。
    const oldSdata = `<?xml version="1.0"?>
<Scenario>
  <Units>
    <Unit>
      <ObjectHandle>1</ObjectHandle>
      <Name>BT_J-1</Name>
      <ModelData><机型><BTAirToMCog uuid="1" type="Cognition"/></机型></ModelData>
      <Script></Script>
    </Unit>
  </Units>
</Scenario>`;
    // 先挂 BT
    const r1 = attachOldScenario(oldSdata, "BT_J-1", "air_to_m", "overwrite");
    expect(r1.ok).toBe(true);
    // 再挂 FSM (同 Unit)
    const r2 = attachOldScenarioStateMachine(r1.xml!, "BT_J-1", "fight_status", "overwrite");
    expect(r2.ok).toBe(true);
    // 两条命令同居 <Script>
    expect(r2.xml).toMatch(/ADD Behaviac "air_to_m";/);
    expect(r2.xml).toMatch(/ADD StateMachine "fight_status";/);
    // 顺序无关,但两条都在同一个 <Script>
    expect(r2.xml).toMatch(/<Script>[\s\S]*ADD Behaviac[\s\S]*ADD StateMachine[\s\S]*<\/Script>/);
    // reject 重复
    const dupFsm = attachOldScenarioStateMachine(r2.xml!, "BT_J-1", "fight_status", "reject");
    expect(dupFsm.ok).toBe(false);
    expect(dupFsm.conflict).toBe(true);
  });
